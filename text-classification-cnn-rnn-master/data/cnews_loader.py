...
def read_file(filename):
    ...
    with open_file(filename) as f:
        for line in f:
            try:
                content, num ,label = line.strip().split('\t')
                if content:
...
def build_vocab(train_dir, vocab_dir, vocab_size=5000):
    ...
    counter = Counter(all_data)
    count_pairs = counter.most_common(vocab_size - 1)
    all_data, _ = list(zip(*count_pairs))
    # print(all_data[:200])

    all_data = ['<PAD>'] + list(all_data)
    open_file(vocab_dir, mode='w').write('\n'.join(all_data) + '\n')
...
def read_category():
    """读取分类目录，固定"""
    # categories = ['Indicator', 'Monetary', 'Option', 'Percentage', 'Product Number', 'Quantity', 'Temporal']
    # categories = ['exercise price', 'maturity date']
...
def process_file(filename, word_to_id, cat_to_id, max_length=128):
    """将文件转换为id表示"""
    ...
    with open_file(filename) as f:
        for line in f:
            try:
                content = line.strip()
                if content:
                    contents.append(native_content(str(content).replace("\n", " ").replace("  ", " ").replace("\"", "").replace("?", "")))
            except:
                pass
    return contents, labels

def process_file2(filename, word_to_id, cat_to_id, max_length=128):
    """将文件转换为id表示"""
...
#     word2vec_embeddings = np.array([[float(v) for v in line.strip().split(' ')] for line in
#                                     open(embedding_file, 'r', encoding='utf-8').readlines()], dtype=np.float32)
#     print(word2vec_embeddings)
#     # 每个向量的维度
#     embedding_size = word2vec_embeddings.shape[1]
#     # 第一个参数：分布的均值 第二个参数：标准差 第三个参数：大小
...
def load_embedding_from_file(embedding_file):
    ...
    return word_dict, np.asarray(embedding_matrix[0], dtype=np.float32)