...
class BiLSTMAttenConfig(object):
    """CNN配置参数"""
...
class BiLSTMAtten(object):
    ...
    def bilstm(self):
        ...
        # outputs = tf.split(self.embedding_inputs, 2, -1)
        #
        ...
        #     output_size = self.config.hiddenSize
        with tf.name_scope('attention'):

            hidden_size = self.embedding_inputs.shape[2].value
            ...
            v = tf.tanh(tf.tensordot(self.embedding_inputs, attention_w, axes=1) + attention_b)
            vu = tf.tensordot(v, attention_u, axes=1, name='vu')
            alphas = tf.nn.softmax(vu, name='alphas')
            attention_output = tf.reduce_sum(self.embedding_inputs * tf.expand_dims(alphas, -1), 1)
        ...
        with tf.name_scope("score"):
            ...
            logits_softmax = tf.nn.softmax(self.logits)
            # fc = tf.layers.dense(output, self.config.hidden_dim, name='fc1')
            # fc = tf.contrib.layers.dropout(fc, self.keep_prob)
            # fc = tf.nn.relu(fc)

            # 分类器
            # logits_softmax = tf.layers.dense(output, self.config.num_classes, name='fc2')
            self.y_pred_cls = tf.argmax(logits_softmax, 1)  # 预测类别
    ...
    def _attention(self, H):
        ...
        r = tf.matmul(tf.transpose(H, [0, 2, 1]), tf.reshape(alpha, [-1, self.config.seq_length, 1]))
